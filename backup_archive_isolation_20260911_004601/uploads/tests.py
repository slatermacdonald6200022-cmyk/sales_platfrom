...
class Normalize1CFileTests(SimpleTestCase):
    def test_reads_new_august_layout_with_product_code(self):
        workbook = Workbook()
        sheet = workbook.active
        sheet.append(['Период: 01.08.2026 - 31.08.2026'])
        for _ in range(6):
            sheet.append([])
        sheet.append(['Клиент', '', '', '', '', '', '', 'Количество'])
        sheet.append(['Заказ клиента'])
        sheet.append(['Номенклатура', '', '', '', 'Артикул товара', '', 'Код'])
        sheet.append(['КЛИЕНТ А ООО', '', '', '', '', '', '', 20])
        sheet.cell(sheet.max_row, 1).alignment = Alignment(indent=0)
        sheet.append(['Заказ клиента №1', '', '', '', '', '', '', 20])
        sheet.cell(sheet.max_row, 1).alignment = Alignment(indent=2)
        sheet.append(['Товар А', '', '', '', '4494450600', '', 'ЦБ-00001234', 20])
        sheet.cell(sheet.max_row, 1).alignment = Alignment(indent=4)
        ...
        self.assertEqual(result.iloc[0]['Клиент'], 'КЛИЕНТ А ООО')
        self.assertEqual(result.iloc[0]['Артикул'], '4494450600')
        self.assertEqual(result.iloc[0]['Код товара'], 'ЦБ-00001234')
        self.assertEqual(result.iloc[0]['Факт, шт'], 20)

    def test_quantity_only_export_preserves_matching_fields(self):
        workbook = Workbook()
        sheet = workbook.active
        sheet.append(['Период: 01.09.2026 - 30.09.2026'])
        sheet.append(['Номенклатура', 'Артикул', 'Количество', 'Класс товара', 'Производственный индекс'])
        sheet.append(['Товар', 'A-1', 12, 'Class', 'Index'])
        sheet.cell(3, 1).alignment = Alignment(indent=2)
        sheet.append(['Клиент', None, 12])
        sheet.cell(4, 1).alignment = Alignment(indent=4)
        uploaded = workbook_upload(workbook)
        self.assertEqual(validate_actual_file(uploaded).period_label, '09.2026')
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / 'quantity.xlsx'
            path.write_bytes(uploaded.read())
            result = normalize_1c_file(path)
        self.assertEqual(result.iloc[0]['Факт, шт'], 12)
        self.assertEqual(result.iloc[0]['Класс товара'], 'Class')
        self.assertEqual(result.iloc[0]['Производственный индекс'], 'Index')
        self.assertNotIn('Факт, CNY', result.columns)
    def test_reads_article_and_client_from_hierarchy_levels(self):
        ...
        self.assertNotIn('Факт, CNY', row.index)
...
class ExcelUploadValidationTests(SimpleTestCase):
    def test_august_multiline_header_ignores_order_date_from_july(self):
        workbook = Workbook()
        sheet = workbook.active
        sheet.append(['Параметры:', '', 'Период: 01.08.2026 - 31.08.2026'])
        for _ in range(6):
            sheet.append([])
        sheet.append(['Клиент', '', '', '', '', '', '', 'Количество'])
        sheet.append(['Заказ клиента'])
        sheet.append(['Номенклатура', '', '', '', 'Артикул товара', '', 'Код'])
        sheet.append(['КЛИЕНТ А ООО'])
        sheet.append(['Заказ клиента от 21.07.2026'])
        sheet.append(['Товар А', '', '', '', 'A-1', '', 'CODE-1', 5])
        result = validate_actual_file(workbook_upload(workbook))
        self.assertEqual(result.period_label, '08.2026')
...
class PeriodScopedMergeTests(SimpleTestCase):
    ...
    def test_unmatched_fact_is_reported_without_entering_result(self):
        ...
        self.assertEqual(report['unmatched_amount_cny'], 0)
        self.assertEqual(
...
class RoleAccessTests(TestCase):
    ...
    def test_role_matrix(self):
        ...
        self.assertTrue(can_view_company_dashboard(self.manager))
    ...
    def test_manager_cannot_download_combined_file(self):
        self.client.force_login(self.manager)
        self.assertEqual(self.client.get(reverse('download_combined_excel')).status_code, 403)

    def test_currency_selector_is_removed(self):
        self.client.force_login(self.analyst)
        response = self.client.get(reverse('upload_files'))
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, 'sourceCurrency')
        self.assertNotContains(response, 'set_1c_currency')
        self.assertContains(response, 'Денежные суммы выгрузки не используются')
    ...
    def test_manager_dashboard_contains_company_data(self):
        rows = [
            {
                'AOP, CNY': 100.0,
                'Прогноз, CNY': 90.0,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(set(json.loads(response.context['dyn_managers'])), {'Редько Вадим', 'Хуснутдинов А.'})
        self.assertEqual(response.context['kpi']['month_fact'], '1 000 079 ¥')
    ...
    def test_dashboard_uses_period_saved_by_latest_processing(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['selected']['period'], ['2027-01'])
